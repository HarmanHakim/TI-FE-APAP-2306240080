...
const getBookingButtonText = (flightId: string) => {
  if (bookingMode.value === 'one-way') {
    ...
    } else if (selectedDepartureFlight.value === flightId) {
      return 'Selected as Departure'
    } else if (!selectedReturnFlight.value) {
      return 'Select Return'
    } else if (selectedReturnFlight.value === flightId) {
      return 'Selected as Return'
    } else {
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex items-center justify-between">
        <div class="flex gap-3">
          <button @click="setBookingMode('one-way')" :class="[
          ...
          <button @click="setBookingMode('two-way')" :class="[
            'px-6 py-2 rounded-lg font-medium transition-all',
            bookingMode === 'two-way'
              ? 'bg-blue-600 text-white shadow-md'
              : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
      ...
      <div v-for="flight in filteredFlights" :key="flight.id" class="bg-white rounded-lg shadow overflow-hidden">
          ...
          <div class="flex items-center justify-between">
            <div class="flex-1 grid grid-cols-1 md:grid-cols-6 gap-4">
              ...
              <div>
                ...
                <span v-if="flight.isDeleted"
                  class="mt-1 inline-block px-2 py-0.5 text-xs bg-red-100 text-red-800 rounded">
                  Deleted
                </span>
                <span v-if="selectedDepartureFlight === flight.id"
                  class="mt-1 inline-block px-2 py-0.5 text-xs bg-green-100 text-green-800 rounded">
                  Departure
                </span>
                <span v-if="selectedReturnFlight === flight.id"
                  class="mt-1 inline-block px-2 py-0.5 text-xs bg-purple-100 text-purple-800 rounded">
                  Return
                </span>
              </div>
            ...
            <div class="ml-4">
              <svg class="w-6 h-6 text-gray-400 transition-transform duration-200"
                :class="{ 'rotate-180': isExpanded(flight.id) }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-show="isExpanded(flight.id)" class="border-t border-gray-200 bg-gray-50 p-6">
            ...
            <div class="bg-white p-4 rounded-lg">
              ...
              <div class="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <p class="font-medium text-gray-900">
                    {{ Math.floor((new Date(flight.arrivalTime).getTime() - new Date(flight.departureTime).getTime()) /
                      (1000 * 60 * 60)) }}h
                    {{ Math.floor(((new Date(flight.arrivalTime).getTime() - new Date(flight.departureTime).getTime()) %
                      (1000 * 60 * 60)) / (1000 * 60)) }}m
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="mt-6 flex flex-wrap gap-3">
            <button @click.stop="router.push(`/flights/${flight.id}`)"
            ...
            <button @click.stop="selectFlightForBooking(flight.id)"
              ...
              {{ getBookingButtonText(flight.id) }}
            </button>
          </div>
        </div>
      </div>
